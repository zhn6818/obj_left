...
ObjLeftDetect::ObjLeftDetect(cv::Mat input, int set_MOG_LearnFrame, int set_min_area, int set_buffer_len, cv::Mat mask)
{
...
void ObjLeftDetect::initialize()
{
	...
	for (int k = 0; k < new_width; k++)
    {
        ...
        memset(image[k], 0, new_height*sizeof(int));
    }
...
//
//
bool ObjLeftDetect::soft_validation3( cv::Mat ImgSynopsis, vector<Obj_info*> obj_left)
{
...
//		IplImage * fg = cvCreateImage(cvSize(new_width,new_height),8,1);
        ...
        cv::Mat fg = cv::Mat(cv::Size(new_width,new_height), CV_8UC3);
...
//	IplImage * temp = cvCreateImage(cvSize(ImgSynopsis->width,ImgSynopsis->height),8,3);
    cv::Mat temp = cv::Mat(cv::Size(ImgSynopsis.cols,ImgSynopsis.rows), CV_8UC3);
	//myImage_2_opencv(ImgSynopsis,temp);
    ImgSynopsis.copyTo(temp);
	cv::imshow("synopsis",temp);
//	cvReleaseImage(&temp);
    B.setTo(0);
//	cvZero(B);
...
//	IplImage * temp2 = cvCreateImage(cvSize(ImgSynopsis->width,ImgSynopsis->height),8,3);
    cv::Mat temp2 = cv::Mat(cv::Size(ImgSynopsis.cols,ImgSynopsis.rows), CV_8UC3);
