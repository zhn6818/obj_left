...
pair<int, int> path_Tag(vector<cv::Point> path, float reserveRatio)
{
	pair<int, int> tag;
...
float path_Length(vector<cv::Point> path)
{
	float distance = 0.0;
...
{
	cv::Scalar pt;
	pt.val[0] = ((uchar *)(img.data + i*img.step))[j*img.channels() + 0];
	pt.val[1] = ((uchar *)(img.data + i*img.step))[j*img.channels() + 1];
	pt.val[2] = ((uchar *)(img.data + i*img.step))[j*img.channels() + 2];
...
{
    ((uchar *)(img.data + i*img.step))[j*img.channels() + 0] = pt.val[0]; // B
    ((uchar *)(img.data + i*img.step))[j*img.channels() + 1] =pt.val[1]; // G
    ((uchar *)(img.data + i*img.step))[j*img.channels() + 2] =pt.val[2]; // R
}

void Set2D_ADD(cv::Mat img ,int i, int j, cv::Scalar pt)
{

	if (((uchar *)(img.data + i*img.step))[j*img.channels() + 0] + pt.val[0] >255)
	{
		((uchar *)(img.data + i*img.step))[j*img.channels() + 0] = 255;
	}
	else
	{
		((uchar *)(img.data + i*img.step))[j*img.channels() + 0]+=pt.val[0]; // B
	...
	if (((uchar *)(img.data + i*img.step))[j*img.channels() + 1] + pt.val[1] >255)
	{
		((uchar *)(img.data + i*img.step))[j*img.channels() + 1] = 255;
	}
	else
	{
		((uchar *)(img.data + i*img.step))[j*img.channels() + 1]+=pt.val[1]; // G
	...
	if (((uchar *)(img.data + i*img.step))[j*img.channels() + 2] + pt.val[2] >255)
	{
		((uchar *)(img.data + i*img.step))[j*img.channels() + 2] = 255;
	}
	else
	{
		((uchar *)(img.data + i*img.step))[j*img.channels() + 2]+=pt.val[2]; // R
...
vector<int> path_resample_id(std::vector<cv::Point> path, float reserveRatio, int reLength)
{
	...
	vector<cv::Point2f> resample;
    cv::Point2f firstPoint;
	firstPoint.x = (float)(path.at(tag.first)).x;
	firstPoint.y = (float)(path.at(tag.first)).y;
	...
	for(int i = tag.first; i<tag.second; i++)
	{
		...
		{
			o1.x = (float)(path.at(i-1)).x;
			o1.y = (float)(path.at(i-1)).y;
		}
		o2.x = (float)(path.at(i)).x;
		o2.y = (float)(path.at(i)).y;
	...
	if (resample.size() == reLength) {
        cv::Point2f p;
		p.x = (float)(path.at(tag.second)).x;
		p.y = (float)(path.at(tag.second)).y;
...
float findTheta(cv::Point p1, cv::Point p2)
{
	double dx = p2.x - p1.x;
	double dy = p2.y - p1.y;
